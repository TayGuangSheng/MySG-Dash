...
function StatusStrip({ busStops }: StatusStripProps) {
  ...
  return (
    <div className="flex min-w-0 flex-wrap items-center justify-center gap-[clamp(12px,1.1vw,18px)] rounded-full bg-white/10 px-[clamp(20px,1.8vw,32px)] py-[clamp(8px,0.9vw,12px)] text-[clamp(12px,1.4vw,16px)] text-white/85 backdrop-blur">
      <StatusItem label="Weather" value={weather?.updatedAt ? formatSingaporeTime(weather.updatedAt) : "--:--"} />
      <Dot />
      <StatusItem label="MRT" value={trains?.updatedAt ? formatSingaporeTime(trains.updatedAt) : "--:--"} />
      <Dot />
      <StatusItem label={`Bus ${firstStop.label}`} value={formatBusSummary(firstBus)} />
      <Dot />
      <StatusItem label={`Bus ${secondStop.label}`} value={formatBusSummary(secondBus)} />
    </div>
  );
}
...
function formatBusSummary(bus: BusApiResponse | undefined) {
